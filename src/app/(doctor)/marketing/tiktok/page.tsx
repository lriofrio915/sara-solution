...
function AIImage({ prompt }: { prompt: string }) {
  ...
  const src = `https://image.pollinations.ai/prompt/${encoded}?width=607&height=1080&nologo=true&seed=${seed}`
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-700 max-w-[130px]" style={{ aspectRatio: '9/16' }}>
        ...
        {status === 'error' && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-1.5 px-2">
            ...
            <button onClick={retry} className="text-[10px] px-2 py-0.5 rounded bg-teal-500 text-white">Reintentar</button>
          </div>
        )}
      ...
      <div className="flex gap-2">
        <button onClick={retry} className="text-xs text-gray-400 hover:text-teal-500">Nueva imagen</button>
        {status === 'ok' && (
          <a href={src} download="tiktok-thumbnail.jpg" target="_blank" rel="noopener noreferrer"
            className="text-xs text-teal-500 hover:underline">Descargar</a>
        )}
      </div>
    </div>
  )
