...
function AIImage({ prompt, aspect }: { prompt: string; aspect: string }) {
  ...
  const src = `https://image.pollinations.ai/prompt/${encoded}?width=${w}&height=${h}&nologo=true&seed=42`
  ...
  return (
    <div className="space-y-2">
      ...
      <a href={src} download="instagram-post.jpg" target="_blank" rel="noopener noreferrer"
        className="text-xs text-pink-600 hover:underline">Descargar imagen</a>
    </div>
  )
}
